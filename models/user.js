...
const UserSchema = new Schema({
    ...
    salt: { type: String }, password: { type: String }, passwordHashVersion: { type: String, enum: ["legacy-hmac-sha256", "bcrypt"], default: "bcrypt" },
    googleId: { type: String, unique: true, sparse: true }, profileImageURL: { type: String, default: "/imgs/default.png" }, bio: { type: String, default: "", maxlength: 500 }, website: { type: String, default: "" }, location: { type: String, default: "", maxlength: 120 },
    role: { type: String, enum: ["USER", "ADMIN"], default: "USER" }, theme: { type: String, enum: ["light", "dark", "system"], default: "system" }, isPrivate: { type: Boolean, default: false }, isDeactivated: { type: Boolean, default: false },
...
UserSchema.pre("save", async function (next) {
    ...
    try { this.password = await bcrypt.hash(this.password, BCRYPT_ROUNDS); this.passwordHashVersion = "bcrypt"; this.salt = undefined; next(); }
...
function legacyPasswordMatches(password, user) {
    ...
    const a = Buffer.from(provided, "utf8"), b = Buffer.from(user.password, "utf8");
    return a.length === b.length && timingSafeEqual(a, b);
}

UserSchema.static("verifyPassword", async function(user, password) {
    if (!user?.password || user.isDeactivated) return false;
    if (user.passwordHashVersion === "bcrypt" || String(user.password).startsWith("$2")) return bcrypt.compare(password, user.password);
    const valid = legacyPasswordMatches(password, user);
    if (valid) { user.password = password; await user.save(); }
    return valid;
});
...
UserSchema.static("matchPassword", async function (email, password) {
    ...
    if (!(await this.verifyPassword(user, password))) throw new Error("Incorrect Password");
...
UserSchema.static("findOrCreateGoogleUser", async function (profile) {
    ...
    try { return await this.create({ fullName: displayName, email, googleId, profileImageURL: photo || "/imgs/default.png" }); }
    catch (error) { if (error?.code === 11000) { const existing = await this.findOne({ $or: [{ googleId }, { email }] }); if (existing) { if (existing.isDeactivated) throw new Error("Account is deactivated"); if (!existing.googleId) { existing.googleId = googleId; if (photo) existing.profileImageURL = photo; await existing.save(); } return existing; } } throw error; }
});
...
UserSchema.methods.isFollowing = function(userId) { return this.following.some(id => id.toString() === id.toString()); };
