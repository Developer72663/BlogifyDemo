...
router.post("/:id/like",async(req,res)=>{try{const blog=await Blog.findById(req.params.id);if(!blog||blog.isDeleted||blog.status!=="published")return res.status(404).json({success:false,message:"Blog not found"});if(!(await canAccessBlog(blog,req.user._id)))return res.status(403).json({success:false,message:"Only followers can like this private blog"});const userId=req.user._id.toString();const hasLiked=blog.likes.some(id=>id.toString()===userId);if(hasLiked)blog.likes=blog.likes.filter(id=>id.toString()!==userId);else blog.likes.push(req.user._id);await blog.save();if(!hasLiked&&blog.createdBy.toString()!==userId){try{await NotificationService.createNotification(blog.createdBy,"like",{title:"New like",message:`${req.user.fullName} liked your blog`,blog:blog._id,actor:req.user._id});}catch(notificationError){console.error("Blog like notification error:",notificationError.message);}}res.json({success:true,liked:!hasLiked,likeCount:blog.likes.length});}catch(error){console.error("Blog like error:",error.message);res.status(500).json({success:false,message:"Failed to like blog"});}});
