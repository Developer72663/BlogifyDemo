...
function styles(){
  ...
  s.id='blogify-message-enhancement-style';
  s.textContent=`
...
function profiles(root){
  (root||document).querySelectorAll('.profile-card:not([data-blogify-enhanced])').forEach(card=>{
    ...
    if(!body.querySelector('.blogify-profile-meta')){
      ...
      st.innerHTML=mutual||(following&&followedBy)?'<i class="fas fa-user-group"></i> Mutual':following?'<i class="fas fa-check"></i> Following':followedBy?'<i class="fas fa-user-plus"></i> Follows you':'<i class="fas fa-user"></i> Profile';
      meta.appendChild(st);body.appendChild(meta);
    }
    const id=profileId(card);
    if(!id)return;
    ...
    if(!body.querySelector('.blogify-profile-action')){
      const a=document.createElement('a');a.className='blogify-profile-action';a.href=href;a.innerHTML='<span>View profile</span><i class="fas fa-chevron-right"></i>';body.appendChild(a);
    }
    if(!card.dataset.profileNavigationBound){
      card.dataset.profileNavigationBound='1';
      card.addEventListener('click',e=>{if(e.target.closest('a,button'))return;location.href=href;});
      card.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();location.href=href;}});
    }
  });
}
...
function longPress(){
  document.querySelectorAll('.message-bubble[data-message-id]:not([data-blogify-hold])').forEach(b=>{
    b.dataset.blogifyHold='1';
    let timer=null,fired=false,x=0,y=0;
    const clear=()=>{if(timer)clearTimeout(timer);timer=null;};
    const isProfileTarget=e=>!!e.target.closest('.profile-card.blogify-shared-profile,.blogify-profile-action');
    b.addEventListener('pointerdown',e=>{
      if(isProfileTarget(e))return;
      if(e.pointerType==='mouse'&&e.button!==0)return;
      fired=false;x=e.clientX;y=e.clientY;clear();
      timer=setTimeout(()=>{
        timer=null;fired=true;
        b.classList.add('blogify-longpress');
        setTimeout(()=>b.classList.remove('blogify-longpress'),220);
        b.click();
      },HOLD_MS);
    },{passive:true});
...
function voiceGuard(){
  ...
  if(!b||b.dataset.blogifyVoiceGuard)return;
  b.dataset.blogifyVoiceGuard='1';
...
function boot(){
  styles();profiles(document);longPress();voiceGuard();
  const o=new MutationObserver(()=>{profiles(document);longPress();voiceGuard();});
