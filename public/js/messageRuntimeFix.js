(function(){
  ...
  if(typeof nativeFetch === 'function'){
    window.fetch = async function(){
      ...
      try{
        ...
        if(url.includes('/messages/')&&!url.includes('/profile-meta/')) response.clone().json().then(rememberPayload).catch(()=>{});
      }catch(_){ }
      return response;
    };
  }
    ...
    window.io=function(){
      ...
      try{
        ...
        socket.on=function(event,handler){
          if(event==='message:new'&&typeof handler==='function') return nativeOn(event,function(message){rememberMessage(message);return handler.apply(this,arguments);});
  ...
  if(NativeRecorder&&!window.__BLOGIFY_MEDIA_RECORDER_PATCHED){
    ...
    const Recorder=function(stream,options){
      ...
      const proxy=new Proxy(recorder,{
        ...
        set(target,prop,value){
          if(prop==='ondataavailable'){
            userDataHandler=typeof value==='function'?function(event){if(window.__BLOGIFY_SUPPRESS_VOICE_DATA)return;return value.call(proxy,event);}:value;
      ...
      proxy.addEventListener('stop',()=>{clearTimeout(autoStop);if(window.__BLOGIFY_ACTIVE_RECORDER===proxy)window.__BLOGIFY_ACTIVE_RECORDER=null;window.__BLOGIFY_CANCEL_RECORDING=false;},{once:true});
      return proxy;
    };
  ...
  document.addEventListener('click',function(event){
    ...
    if(cancel){window.__BLOGIFY_CANCEL_RECORDING=true;setTimeout(()=>{window.__BLOGIFY_CANCEL_RECORDING=false;},1600);}
  },true);

  function applyStatus(card,p){
    const status=card.querySelector('.blogify-follow-status');
    if(!status)return;
    ...
    status.innerHTML=mutual||(following&&followedBy)?'<i class="fas fa-user-group"></i> Mutual':following?'<i class="fas fa-check"></i> Following':followedBy?'<i class="fas fa-user-plus"></i> Follows you':'<i class="fas fa-user"></i> Profile';
  }
  ...
  async function loadMeta(card,profile){
    ...
    try{
      ...
      profile.isFollowing=d.profile.isFollowing;
      profile.followsYou=d.profile.followsYou;
      profile.isMutual=d.profile.isMutual;
      applyStatus(card,profile);
      card.appendChild(document.createComment('profile-meta'));
    }catch(_){ }
  }
