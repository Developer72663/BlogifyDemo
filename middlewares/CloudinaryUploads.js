...
function getConfigOrReject(type) {
    const config = getCloudinaryConfig();
    if (!config.cloud_name || !config.api_key || !config.api_secret) {
        return Promise.reject(new Error(`${type} upload is not configured on the server. Set CLOUD_NAME, API_KEY and API_SECRET.`));
    }
...
// Cloudinary trims the uploaded video to its first 60 seconds in the generated
// delivery asset. The original upload is not used by Blogify after this step.
function uploadVideoBuffer(buffer, options = {}) {
    if (!buffer || !Buffer.isBuffer(buffer)) return Promise.reject(new Error('No video data received'));
    getConfigOrReject('Video');
    return new Promise((resolve, reject) => {
        const uploadOptions = {
...
mediaUpload.uploadBuffer = uploadBuffer;
mediaUpload.uploadVideoBuffer = uploadVideoBuffer;
module.exports = cloudinaryUpload;
module.exports.mediaUpload = mediaUpload;
module.exports.uploadVideoBuffer = uploadVideoBuffer;
